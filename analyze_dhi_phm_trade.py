#!/usr/bin/env python3
"""
Workflow 2 – DHI/PHM Trade Readiness Engine

- Data: DHI=y, PHM=x, start-date=2018-01-01, auto-adjust ON
- Beta-neutral (OLS) + cash spread
- Metrics: Beta_OLS, Beta_60/90, Beta stability %, Spread_beta, Spread_cash
- ADF test, Half-life, Z60/Z30, Vol_ratio
- Hedge recommendation: beta_neutral / cash_neutral
- Entry criteria & TRADE_READY = YES/NO
- Output CSV/JSON, optional PNG chart
- CLI flags: --use-percent-returns, --winsorize/--no-winsorize, --plot/--no-plot
- --help-checklist prints checklist and exits
"""
...
def download_data(tickers, start_date):
    df = yf.download(tickers, start=start_date, progress=False, auto_adjust=True, threads=1)
...
def rolling_beta(y, x, window):
    return y.rolling(window).cov(x) / x.rolling(window).var().iloc[-1]
...
def main():
    ...
    # Returns
    rets = compute_returns(df, args.use_percent_returns, args.winsorize)
    ret_y = rets["DHI"]
    ret_x = rets["PHM"]

    # OLS
    df_ols = pd.concat([y,x], axis=1).dropna()
    X = sm.add_constant(df_ols["PHM"])
    ols_model = OLS(df_ols["DHI"], X).fit()
    ...
    if args.plot:
        ...
        plt.subplot(3,1,2)
        zscore(spread_beta,60).plot(label="Z60")
        zscore(spread_beta,30).plot(label="Z30")
        plt.axhline(2,color="red",linestyle="--"); plt.axhline(1.5,color="orange",linestyle="--")
        plt.axhline(-1.5,color="orange",linestyle="--"); plt.axhline(-2,color="red",linestyle="--")
        plt.axhline(0.5,color="grey",linestyle=":"); plt.axhline(-0.5,color="grey",linestyle=":")
    ...
    for k,v in output.items():
        print(f"{k:20s}: {v}" if not isinstance(v,float) else f"{k:20s}: {v:.4f}")
    print(f"\nTRADE_READY = {trade_ready}")
    if trade_ready=="YES":
